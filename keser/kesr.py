...
async def search_username(username, save_file=None, search_all=False, print_summary=True, top_sites=False, timeout=26.8):
    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        ...
        # Create loading bar with context manager
        with LoadingBar(len(sites_to_check), "Searching") as loading_bar:
            if top_sites:
            ...
            for task in asyncio.as_completed(tasks):
                ...
                if result and result[0] not in unique_sites:
                    ...
                    found.append(result)
                    loading_bar.update(1, 1)
                else:
                    loading_bar.update(1, 0)
...
def process_brute_force_duckduckgo(usernames_input, save_file=None, max_retries=2):
    ...
    # Create loading bar with context manager
    with LoadingBar(len(usernames), "Processing usernames") as loading_bar:
        for username in usernames:
            ...
            loading_bar.update(1, username_links)
            time.sleep(4.3)
